...
def download_job(job_id, url, start_time=None, end_time=None):
    ...
    cmd = [
        ...
        *(["--cookies", str(COOKIES_FILE), "--extractor-args", "youtube:player_client=ios", "--format", "bestaudio/best"] if COOKIES_FILE.exists() else ["--extractor-args", "youtube:player_client=android,tv_embedded", "--js-runtimes", f"node:{node_path}", "--user-agent", "Mozilla/5.0 (Linux; Android 11; Pixel 5) AppleWebKit/537.36"]),
